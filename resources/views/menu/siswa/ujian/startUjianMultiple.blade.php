    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.addEventListener("fullscreenchange", function() {
                if (!document.fullscreenElement) {
                    ...
                    window.location.href =
                        "{{ route('selesaiUjianMultiplev2', ['token' => encrypt($userCommit['id'])]) }}"; // Redirect ke halaman finish ujian
                }
            });
            ...
            document.addEventListener("visibilitychange", function() {
                if (document.hidden && !modalSelesai) {
                    alert('Anda telah keluar dari tab ujian. Sesi ujian akan diakhiri');
                    window.location.href = "{{ route('selesaiUjianMultiplev2', ['token' => encrypt($userCommit['id'])]) }}";
                }
            });
