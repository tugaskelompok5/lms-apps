...
class PengajarController extends Controller
{
    ...
    public function viewKepsek()
    {
        ...
        return view('menu.admin.controlKepsek.viewKepsek', ['title' => 'Data Kepala Sekolah', 'roles' => $roles, 'pengajar' => User::where('roles_id', 4)->paginate(15)]);
    }
    ...
    public function viewNewKepsek()
    {
        $roles = DashboardController::getRolesName();

        return view('menu.admin.controlKepsek.tambahKepsek', ['title' => 'Tambah Kepala Sekolah', 'roles' => $roles]);
    }
    ...
    public function validateDataKepsek(Request $request)
    {
        // Validasi inputan form
        $request->validate([
            'nama' => 'required|min:5',
        ...
        return redirect(route('viewKepsek'));
    ...
    public function viewUpdateKepsek($token)
    {
        try {
            $id = Crypt::decrypt($token);
            $roles = DashboardController::getRolesName();
            ...
            return view('menu.admin.controlKepsek.updateKepsek', ['user' => $profile, 'contact' => $contact, 'kelas' => $kelas, 'mapelEnrolled' => $mapelEnrolled, 'roles' => $roles, 'title' => 'Update Kepala Sekolah']);
        } catch (\Illuminate\Contracts\Encryption\DecryptException $e) {
            abort(404);
        }
    }
    ...
    public function destroyKepsek(Request $request)
    {
        // dd($request);
        User::where('id', $request->idHapus)->delete();
        EditorAccess::where('user_id', $request->idHapus)->delete();

        return redirect(route('viewKepsek'))->with('delete-success', 'Berhasil menghapus data!');
    }
    ...
    {
        $request->validate([
            'file' => 'required|mimes:xlsx', // Sesuaikan dengan jenis file Excel yang diizinkan
        ]);
    ...
    public function fileContoh()
    {
        ...
        $file = public_path() . '/examples/formatpengajarsmk6.xlsx';

        return response()->download($file, 'contoh-data-pengajar.xlsx');
    }
}
