...
class UjianController extends Controller
{
    ...
    public function selesaiUjianMultiple(Request $request)
    {
        ...
        $ujianss = SoalUjianMultiple::where('ujian_id', $ujian->id)->get();
        $countujian = count($ujian->SoalUjianMultiple);
        
        // dd($countUjian);
        $jawabanuser = UserJawaban::where('multiple_id', $ujianss[0]->id)->where('user_id', Auth()->User()->id)->get();
    ...
    public function selesaiUjianMultiplev2($token)
    {
        $id = decrypt($token);
